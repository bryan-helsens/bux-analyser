...
NEEDS_FUNDAMENTALS = "Needs company fundamentals, which are not loaded yet."
...
def build_scores(metric_frame: pd.DataFrame, names: dict[str, str], as_of: date) -> dict[str, Score]:
    ...
    for isin in metric_frame.index:
        ...
        for pillar_key, (label, definitions) in PILLAR_DEFINITIONS.items():
            ...
            if not definitions:
                pillar.unavailable_reason = NEEDS_FUNDAMENTALS
            ...
            if usable:
                ...
                pillar.unavailable_reason = "None of these metrics could be computed for this holding."
            else:
        ...
        missing = [p.label for p in score.pillars.values() if not p.available]
        if missing:
...
def recommend(score: Score, change: ScoreChange | None, weight: float | None,
    ...
    rec.risks.append("Valuation is not assessed: no fundamentals are loaded, so an expensive "
                     "holding cannot be distinguished from a cheap one")
...
def unavailable_labels() -> dict[str, str]:
    """Labels the engine cannot produce yet, and why."""
